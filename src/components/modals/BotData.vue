<template>
  <Modal  @close="close">
    <button class="modal__edit btn btn--edit" @click="openEditBot">
    ...
    <h3 class="modal__headline">{{ activeBot.name }}</h3>
    <div class="modal__bot-avatar bot-avatar info" v-if="activeBot.avatar">
      <img class="bot-avatar__image" :src="activeBot.avatar" alt="" >
    </div>
    <div class="modal__bot-avatar bot-avatar" v-else>
      ?
    </div>
    <span class="modal__bot-date">{{ date }}</span>
    <p class="modal__bot-description">
  ...
  export default {
    ...
    methods: {
      openEditBot() {
        ...
        this.$store.commit('OPEN_EDIT_BOT_MODAL');
      },
      close() {
        this.$emit('close')
      }
    ...
    computed: {
      ...
      date() {
        return moment(this.activeBot.date).format("LLL");
      }
    },
    components: {
      Modal,
